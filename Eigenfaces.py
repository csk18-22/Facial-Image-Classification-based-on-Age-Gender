...
dirname = "models/dataset/bw/jpg2/"
# print(dirname)
count = 0
...
# trainging data is half the aize of total no. of images
data = np.zeros((total//2, size[0]*size[1]))
...
# sizze of weight for trainging data == SIZE(training X k)
wttraining = np.dot(eigenfaces, data.transpose()).transpose()
...
for j in range(len(testf)):
    ...
    # size of weight for teseting data==SIZE(1 X K)
    wttest = np.dot(eigenfaces, testimg.transpose()).transpose()
    # print wttest.shape
    ...
    for i in range(len(data)):
        # Dist of ith-EigenFace with current test data
        cur = np.linalg.norm(wttraining[i, :]-wttest[:])
        if mini > cur:
            mini = cur
            ind = i
